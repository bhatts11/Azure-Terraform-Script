...
# data "azurerm_key_vault_secret" "vmss_user" {
#   name         = var.vmss_Usr
#   key_vault_id = data.azurerm_key_vault.kv_name.id
# }

# data "azurerm_key_vault_secret" "vmss_passwd" {
#   name         = var.vmss_Passwd 
#   key_vault_id = data.azurerm_key_vault.kv_name.id
# }
...
module "vmss" {
  ...
  source                   = "../terraform-modules/vmss"
  depends_on               = [module.vnet01.subnet]
  vmss_pip_prefix          = var.vmss_pip_prefix
  vmss_name                = var.vmss_name
  vmss_sku                 = var.vmss_sku
  vmss_instance            = var.vmss_instance
  vmss_user                = data.azurerm_key_vault_secret.vmss_user.value
  vmss_password            = data.azurerm_key_vault_secret.vmss_passwd.Value
  prefix                   = var.prefix
  ...
  vmss_storage_acc_type    = var.vmss_storage_acc_type
  vmss_os_disk_caching     = var.vmss_os_disk_caching
  vmss_auto_config         = var.vmss_auto_config
  vmss_def_count           = var.vmss_def_count
  vmss_max_count           = var.vmss_max_count
  vmss_min_count           = var.vmss_min_count
  vmss_cpu_upper_threshold = var.vmss_cpu_upper_threshold
  vmss_cpu_lower_threshold = var.vmss_cpu_lower_threshold
  vmss_subnetid            = module.vnet01.subnet_Id[3]
