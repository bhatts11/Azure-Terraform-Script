variable "AKS_LAWName" {
  ...
  validation {
    condition     = length(var.AKS_LAWName) > 6 && can(regex("onef", lower(var.AKS_LAWName)))
    error_message = " The AKS_LAWName must be valied name, shold contain\" onef \" ."
  }
}
variable "AKS_LAWSKU" {
  type        = string
...
variable "AKS_ClusterName" {
  ...
  validation {
    condition     = length(var.AKS_ClusterName) > 6 && can(regex("onef", lower(var.AKS_ClusterName)))
    error_message = " The AKS_ClusterName must be valied name, shold contain\" onef \" ."
  }
}
